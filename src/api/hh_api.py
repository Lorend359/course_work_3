...
def search_employers(query: str, per_page: int = 10) -> List[Dict[str, Any]]:
    ...
    """
    response = requests.get(f"{HH_API_URL}/employers", params={"text": query, "per_page": per_page})
...
def get_vacancies_by_employer(employer_id: int, per_page: int = 100) -> List[Dict[str, Any]]:
    ...
    """
    response = requests.get(f"{HH_API_URL}/vacancies", params={"employer_id": employer_id, "per_page": per_page})
    response.raise_for_status()
    data = response.json()
    return data.get("items", [])
...
def get_top_companies() -> None:
    ...
    for name in company_names:
        ...
        if results:
            emp = results[0]
            companies.append({"id": emp["id"], "name": emp["name"]})
        else:
